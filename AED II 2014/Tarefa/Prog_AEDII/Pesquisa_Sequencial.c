...
typedef struct {
	TItem *Item;
	TIndice n, max;//n=quantidade de itens da tabela e max=quantos elementos eu tenho disponivel para colocar na tabela
} TTabela;
...
TIndice Pesquisa(TTabela *T, TChave x)
{
	...
	for(i=0; i < T->n; i++)//Pesquisa o elemento
		if(T->Item[i].Chave == x)
			return i;
	return -1;//Caso chega-se nesse ponto, eh porque a pesquisa foi sem sucesso
}

/* insere um registro na tabela */
void Insere(TTabela *T, TItem x)
{
	if (T->n == T->max)//verifico se eu tenho espaco disponivel na tabela, e se n for igual a max a tabela esta cheia
	{
		T->max += 2;
		T->Item = (TItem *)realloc(T->Item, T->max * sizeof(TItem));
	}
	
	T->Item[T->n++] = x;//insiro um elemento na posicao n, pois quando chegar aqui eh certeza que tem espaco, e depois soma-se +1
}
