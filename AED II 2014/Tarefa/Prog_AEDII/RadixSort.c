...
void Ordena(TItem *A, int n, int d) // Ajuste o parametro do seu metodo aqui!
{
	...
	for(j=0; j<n; j++)//Vai acessar o conteudo de A e incrementa 1
		C[retornaDigito(A[j].Chave, digito)]++;//Pega o numero da posicao j de A e coloca 1 nessa posicao em C, por exemplo, o 0 fica na posicao 1, logo sera armazenado em C o valor 1, que o local da posicao.

	for(i=1; i <= k; i++)
		C[i] += C[i-1];

	for(j = n-1; j >= 0; j--)
		B[--C[retornaDigito(A[j].Chave, digito)]] = A[j];//Pega o valor de A[j] e coloca no local correto, que sera indicado pelo vetor C. Isso sera: chave recebe o retorno da funcao retornaDigito(A[j].Chave, digito), c[chave] recebe C[chave] - 1, indice recebe C[chave] e B[indice] recebe A[j]. Melhor representacao disso seria:
	/*
